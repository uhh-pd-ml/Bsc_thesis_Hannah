...
for group in groups:
    ...
    # Falls nur ein Index in der Gruppe wäre
    if n == 1: 
        axes = [axes]
    ...
    for i, idx in enumerate(indices):
        ...
        # Histogramme plotten 
        ax.hist(all_features_b[:, idx], bins=40, alpha=0.5, label='Background', color='blue', density=True)
        ax.hist(all_features_s[:, idx], bins=40, alpha=0.5, label='Signal', color='red', density=True)
        
        ax.set_title(f"Feature {idx}: {feature_names[idx]}")
        ax.legend(fontsize='small')
        ax.grid(alpha=0.3)
        #ax.set_yscale('log') # Kann bei Bedarf einkommentiert werden

    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    save_path = os.path.join(output_dir, f"plot_{group['title']}.png")
